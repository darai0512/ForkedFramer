...
export class LayeredCanvas {
  ...
  async handlePaste(event: ClipboardEvent): Promise<void> {
    ...
    let cursor = this.pointerCursor;
    if (cursor == null) {
      const cw = this.viewport.canvas.width;
      const ch = this.viewport.canvas.height;
      const v = this.viewport.canvasPositionToViewportPosition([cw / 2, ch / 2]);
      cursor = this.viewportPositionToRootPaperPosition(v);
    }
    let accepted = false;
